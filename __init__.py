...
def get_angles_and_leds():
    ...
    if scn.scn_prop.SendLEDVals:
        for value in range(0, len(scn.my_list2)):
            ...
            # Neopixels
            if scn.my_list2[value].LEDType == "Neopixel" or scn.my_list2[value].LEDType == "Dotstar":
                isRGBW = False

                if scn.my_list2[value].NeopixelOrder == "NEO_RGBW + NEO_KHZ800" or scn.my_list2[value].NeopixelOrder == "NEO_GRBW + NEO_KHZ800" or scn.my_list2[value].NeopixelOrder == "NEO_RGBW + NEO_KHZ400" or scn.my_list2[value].NeopixelOrder == "NEO_GRBW + NEO_KHZ400":
                    isRGBW = True

                if scn.my_list2[value].LEDSingle:
                    # Send one color value to whole strip
                    if scn.my_list2[value].ArmatureSelect != 'None' and scn.my_list2[value].RedSelect1 != 'None' and scn.my_list2[value].GreenSelect1 != 'None' and scn.my_list2[value].BlueSelect1 != 'None':
                        RGBW = []
                        outputbytes.append(int(scn.my_list2[value].LEDSingle).to_bytes(1,'big')[0])
                ...
                # Send individual LED values using geometry nodes graph
                    ...
                    if scn.my_list2[value].RedSelect2 != 'None' and scn.my_list2[value].GreenSelect2 != 'None' and scn.my_list2[value].BlueSelect2 != 'None':
                        dgEv = bpy.context.evaluated_depsgraph_get()
                        outputbytes.append(int(scn.my_list2[value].LEDSingle).to_bytes(1,'big')[0])
            ...
            # Single color
                ...
                if scn.my_list2[value].ArmatureSelect != "None" and scn.my_list2[value].LEDSelect != "None" and scn.my_list2[value].LEDAngle != "None":
                    rgb_col = math.floor(interp(get_bones_rotation(scn.my_list2[value].ArmatureSelect, scn.my_list2[value].LEDSelect, scn.my_list2[value].LEDAngle), [0,90], [0,255])).to_bytes(1, 'big')
...
class ListItem(bpy.types.PropertyGroup):
    ...
    ShowValueDebug: prop.BoolProperty(
        name = "Show Value",
        description = "Display the angle values",
        default = True)
...
class SendSingle(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if comport != 'None':
            ...
            global outputbytes
            #serialport = serial.Serial('COM3', 9600, timeout = 0)
            serialport = serial.Serial(scn.scn_prop.PortSelect, scn.scn_prop.BaudRate, timeout = 0.001)
            print('Connected to ' + comport + " at " + scn.scn_prop.BaudRate + " baud")
            scn.scn_prop.SerialConnected = True
...
class Sync(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # Overwrite file
        if scn.scn_prop.OverwriteArduinoFile or (not scn.scn_prop.OverwriteArduinoFile and not os.path.exists(filenamez)):
        ...
        if scn.scn_prop.ConfigType == ".h File":
            ...
            for item in range(0, len(scn.my_list2)):
                if scn.my_list2[item].LEDType == "Neopixel" or scn.my_list2[item].LEDType == "Dotstar":
            ...
            # START OF FILE
            outputfile.write("// MarIOnette generated config file for motors and LEDs //\n".encode())
            
            outputfile.write(("#define BAUD_RATE_SERIAL " + str(scn.scn_prop.BaudRate)).encode())
            outputfile.write(('\n\n' + "#define SD_ENABLE " + str(int(scn.scn_prop.SDCardEnabled))).encode())
            outputfile.write(('\n' + "#define USE_SD_INTERNAL " + str(int(scn.scn_prop.SDCardInternal))).encode())
            ...
            for item in range(0, len(scn.my_list)):
                ...
                if scn.my_list[item].MotorType == "Servo":
                    if scn.my_list[item].EnableLimits:
                        outputfile.write(("{1, " + str(scn.my_list[item].ServoPin) + ", 0, 0, 0, 0, 0, 0, " + str(scn.my_list[item].LimitMin) + ", " + str(scn.my_list[item].LimitMax) +  "}").encode())
                    else:
                        outputfile.write(("{1, " + str(scn.my_list[item].ServoPin) + ", 0, 0, 0, 0, 0, 0, 0, 0}").encode())
                elif scn.my_list[item].MotorType == "PWM":
                    if scn.my_list[item].EnableLimits:
                        outputfile.write(("{2, " + str(scn.my_list[item].PWMPin1) + ", 0, 0, 0, 0, 0, " + str(scn.my_list[item].LimitMin) + ", " + str(scn.my_list[item].LimitMax) +  "}").encode())
                    else:
                        outputfile.write(("{2, " + str(scn.my_list[item].PWMPin1) + ", 0, 0, 0, 0, 0, 0, 0, 0}").encode())
                elif scn.my_list[item].MotorType == "ON/OFF":
                    outputfile.write(("{3, " + str(scn.my_list[item].ONOFFPIN) + ", 0, 0, 0, 0, 0, 0, 0, 0}").encode())
                elif scn.my_list[item].MotorType == "Bi-directional PWM":
                    if scn.my_list[item].PWMandDir:
                        if scn.my_list[item].EnableLimits:
                            outputfile.write(("{4, " + str(scn.my_list[item].PWMPin1) + ', ' + str(scn.my_list[item].DirPin) + ", 1, 0, 0, 0, 0, " + str(scn.my_list[item].LimitMin) + ", " + str(scn.my_list[item].LimitMax) +  "}").encode())
                        else:
                            outputfile.write(("{4, " + str(scn.my_list[item].PWMPin1) + ', ' + str(scn.my_list[item].DirPin) + ", 1, 0, 0, 0, 0, 0, 0}").encode())
                    else:
                        if scn.my_list[item].EnableLimits:
                            outputfile.write(("{4, " + str(scn.my_list[item].PWMPin1) + ', ' + str(scn.my_list[item].PWMPin2) + ", 0, 0, 0, 0, 0, " + str(scn.my_list[item].LimitMin) + ", " + str(scn.my_list[item].LimitMax) +  "}").encode())
                        else:
                            outputfile.write(("{4, " + str(scn.my_list[item].PWMPin1) + ', ' + str(scn.my_list[item].PWMPin2) + ", 0, 0, 0, 0, 0, 0, 0}").encode())
                elif scn.my_list[item].MotorType == "Stepper":
                    if scn.my_list[item].EnableLimits:
                        outputfile.write(("{5, " + str(scn.my_list[item].StepPin) + ", " + str(scn.my_list[item].DirPin) + ", " + str(scn.my_list[item].StepperMicrosteps) + ", " + str(scn.my_list[item].Speed) + ", " + str(scn.my_list[item].Acceleration) + ", 0, 0, " + str(scn.my_list[item].LimitMin) + ", " + str(scn.my_list[item].LimitMax) +  "}").encode())
                    else:
                        outputfile.write(("{5, " + str(scn.my_list[item].StepPin) + ", " + str(scn.my_list[item].DirPin) + ", " + str(scn.my_list[item].StepperMicrosteps) + ", " + str(scn.my_list[item].Speed) + ", " + str(scn.my_list[item].Acceleration) + ", 0, 0, 0, 0}").encode())
                elif scn.my_list[item].MotorType == "LewanSoul Bus Servo":
                    if scn.my_list[item].EnableLimits:
                        outputfile.write(("{7, " + str(scn.my_list[item].IDNumber) + ", 0, 0, 0, 0, 0, 0, " + str(scn.my_list[item].LimitMin) + ", " + str(scn.my_list[item].LimitMax) +  "}").encode())
                    else:
                        outputfile.write(("{7, " + str(scn.my_list[item].IDNumber) + ", 0, 0, 0, 0, 0, 0, 0, 0}").encode())
                elif scn.my_list[item].MotorType == "Dynamixel":
                    if scn.my_list[item].EnableLimits:
                        outputfile.write(("{8, " + str(scn.my_list[item].IDNumber) + ", 0, 0, 0, 0, 0, 0, " + str(scn.my_list[item].LimitMin) + ", " + str(scn.my_list[item].LimitMax) +  "}").encode())
                    else:
                        outputfile.write(("{8, " + str(scn.my_list[item].IDNumber) + ", 0, 0, 0, 0, 0, 0, 0, 0}").encode())
            ...
            if len(scn.my_list2) > 0:
                ...
                for item in range(0, len(scn.my_list2)):
                    ...
                    if scn.my_list2[item].LEDType == "Neopixel":
                        ...
                        outputfile.write(("{10, " + str(scn.my_list2[item].DataPin) + ", " + str(scn.my_list2[item].NumLeds) + ", " + str(scn.my_list2[item].NumLeds) + ", " + str(int(scn.my_list2[item].RGBW)) + "}").encode())
                    elif scn.my_list2[item].LEDType == "Dotstar":
                        outputfile.write(("{10, " + str(scn.my_list2[item].DataPin) + ", " + str(scn.my_list2[item].ClockPin) + ", " + str(str(scn.my_list2[item].NumLeds)) + ", " + str(int(scn.my_list2[item].RGBW)) + "}").encode())
                    elif scn.my_list2[item].LEDType == "Single Color":
                        outputfile.write(("{11, " + str(scn.my_list2[item].DataPin) + ", 0, 0, 0}").encode())
            ...
            outputfile.write(("\n#include \"Arduino.h\"\n").encode())
            outputfile.write(("\n// For AVR - based boards without a dedicated Serial buffer, we need to add a small delay between Serial reads").encode())
            outputfile.write(("\n// Teensy appears to use a default 12-bit resolution for Analog writes...").encode())
            ...
            if num_dynamixel > 0:
                ...
                outputfile.write(("\nDynamixel2Arduino dxl(" + str(scn.scn_prop.DynamixelSerialPort) + ", " + str(scn.scn_prop.DynamixelDirPin) + ");\n").encode())
                outputfile.write(("\n// This namespace is required to use Control table item names\nusing namespace ControlTableItem;").encode())
                outputfile.write(("\nint oldDynamixelSpeed = " + str(scn.scn_prop.DynamixelSpeed) + ";").encode())
            ...
            if num_neopixel + num_dotstars > 0:
                ...
                outputfile.write(('\n' + "CRGB neopixels[" + str(num_neopixel + num_dotstars) + "][" + str(max_neopixels) + "];").encode())
            ...
            if len(scn.my_list) > 0:
                ...
                if num_lewansoul > 0:
                    outputfile.write(("\n  // Bus Servos\n  if(motor_values[i][0] == 7){\n   " + str(scn.scn_prop.BusServoSerialPort) + ".begin(BUS_SERVO_BAUD);\n   BusServos.pSerial = &" + str(scn.scn_prop.BusServoSerialPort) + ";\n   Serial.println(\"Connected to bus servo on Serial1\");\n  }").encode())
                
                if num_dynamixel > 0:
                    outputfile.write(("\n  // Dynamixels\n  dxl.begin(" + str(scn.scn_prop.DynamixelBaud) + ");\n  dxl.setPortProtocolVersion(" + str(scn.scn_prop.DynamixelProtocolVer) + ");\n").encode())
                    ...
                    outputfile.write(("\n   dxl.writeControlTableItem(PROFILE_VELOCITY, i, " + str(scn.scn_prop.DynamixelSpeed) + ");\n  }").encode())


                if num_steppers > 0:
                    outputfile.write(("\n\n  // Steppers\n  Serial.println(\"Starting stepper initialization...\");\n").encode())
                    outputfile.write(("  for(int i = 0; i < TOTAL_STEPPERS; i++){\n").encode())
                    outputfile.write(("   for(int j = 0; j < TOTAL_MOTORS; j++){\n").encode())
                    outputfile.write(("    if(motor_values[j][0] == 5){\n").encode())
                    outputfile.write(("     steppers[i] = AccelStepper(steppers[i].DRIVER, motor_values[j][1], motor_values[j][2]);\n").encode())
                    outputfile.write(("     steppers[i].setMaxSpeed(motor_values[j][4]); // 100mm/s @ 80 steps/mm\n").encode())
                    outputfile.write(("     steppers[i].setAcceleration(motor_values[j][5]); // 2000mm/s^2\n").encode())
                    outputfile.write(("     steppers[i].setPinsInverted(false, false, true);\n").encode())
                    outputfile.write(("     steppers[i].enableOutputs();\n     i++;\n    }\n   }\n  }").encode())
        
                outputfile.write(("\n }").encode())

                if len(scn.my_list2) > 0:
                    outputfile.write(("\n\n // PWM LEDs\n for(int i = 0; i < TOTAL_LEDS; i++){").encode())
                ...
                if num_neopixel + num_dotstars > 0:
                    ...
                    for blah in range(0, len(scn.my_list2)):
                        if scn.my_list2[blah].LEDType == "Neopixel":
                            ...
                            outputfile.write(("\n\n FastLED.addLeds<NEOPIXEL, " + str(scn.my_list2[blah].DataPin) +  ">(neopixels[" + str(numz) + "], " + str(max_neopixels) + ");\n").encode())
                            numz+=1
                        elif scn.my_list2[blah].LEDType == "Dotstar":
                            outputfile.write(("\n\n FastLED.addLeds<DOTSTAR, " + str(scn.my_list2[blah].DataPin) + ", " + str(scn.my_list2[blah].ClockPin)+ ", " + str(scn.my_list2[blah].NeopixelOrder) +  ">(neopixels[" + str(numz) + "], " + str(max_neopixels) + ");\n").encode())
                            numz+=1
            ...
            outputfile.write(("\n\nvoid updateMotorsAndLEDs(char frame_buffer[], int mode){").encode())
            if len(scn.my_list) > 0:
                outputfile.write(("\n int servo_index = 0;\n int stepper_index = 0;").encode())
                outputfile.write(("\n\n for(int i = 0; i < TOTAL_MOTORS; i++){").encode())
                outputfile.write(("\n  int offset = i*2;\n\n  // Speed bytes are sent in case we have bus servos...\n  if(mode == 0){\n   offset = i*2+expectedSpeedBytes;\n  }\n\n  unsigned int motor_value = word(frame_buffer[offset], frame_buffer[offset+1]);").encode())
                ...
                if num_dynamixel > 0:
                    outputfile.write(("\n\n  // Set Dynamixel\n  if(motor_values[i][0] == 8){\n   if(busServoSpeed != oldDynamixelSpeed){").encode())
                ...
                outputfile.write(("\n\n  // Set Bi-directional PWM\n  if(motor_values[i][0] == 4){").encode())
                outputfile.write(("\n   if(motor_values[i][3] == 1){").encode())
                outputfile.write(("\n    if(motor_value > ANALOG_MAX/2){").encode())                
                outputfile.write(("\n     analogWrite(motor_values[i][1], map(motor_value, ANALOG_MAX/2, ANALOG_MAX, 0, ANALOG_MAX));").encode())
                outputfile.write(("\n     digitalWrite(motor_values[i][2], HIGH);\n    }").encode())
                outputfile.write(("\n\n    else{").encode())
                outputfile.write(("\n     analogWrite(motor_values[i][1], map(motor_value, ANALOG_MAX/2, 0, 0, ANALOG_MAX));").encode())
                outputfile.write(("\n     digitalWrite(motor_values[i][2], LOW);\n    }\n   }").encode())
                outputfile.write(("\n\n   else{").encode())
                outputfile.write(("\n    if(motor_value > ANALOG_MAX/2){").encode())
                outputfile.write(("\n     analogWrite(motor_values[i][2], map(motor_value, ANALOG_MAX/2, ANALOG_MAX, 0, ANALOG_MAX));").encode())
                outputfile.write(("\n     digitalWrite(motor_values[i][1], LOW);\n    }").encode())
                outputfile.write(("\n\n    else{").encode())
                outputfile.write(("\n     analogWrite(motor_values[i][1], map(motor_value, ANALOG_MAX/2, 0, 0, ANALOG_MAX));").encode())
                outputfile.write(("\n     digitalWrite(motor_values[i][2], LOW);\n    }\n   }\n  }").encode())
            ...
            if len(scn.my_list2) > 0:
                ...
                if num_neopixel + num_dotstars > 0:
                    ...
                    outputfile.write(("\n  else if(led_values[i][0] == 10 && frame_buffer[offset] == 1){").encode())
                    outputfile.write(("\n   offset++;").encode())
                    ...
                    outputfile.write(("\n   else{\n    offset +=3;\n   }\n").encode())
                    outputfile.write(("\n   if(led_values[i][4] == 1){").encode())
                    outputfile.write(("\n    fill_solid(neopixels[neopixel_index], " + str(max_neopixels)+ ", CRGB(red, green, blue));\n   }\n").encode())
                    outputfile.write(("\n   else{\n    fill_solid(neopixels[neopixel_index], " + str(max_neopixels)+ ", CRGB(red, green, blue));\n   }\n").encode())
                    outputfile.write(("\n   FastLED.show();").encode())
                    ...
                    outputfile.write(("\n   for(unsigned int j = 0; j < led_values[i][3]; j++){").encode())
                    outputfile.write(("\n    int red = int(frame_buffer[offset]);").encode())
                    outputfile.write(("\n    int green = int(frame_buffer[offset + 1]);").encode())
                    outputfile.write(("\n    int blue = int(frame_buffer[offset + 2]);\n").encode())
                    outputfile.write(("\n    if(led_values[i][4] == 1){").encode())
                    outputfile.write(("\n     int white = int(frame_buffer[offset + 3]);").encode())
                    outputfile.write(("\n     neopixels[neopixel_index][j].setRGB(red, green, blue);").encode())
                    outputfile.write(("\n     offset += 4;\n    }\n").encode())
                    outputfile.write(("\n    else{").encode())
                    outputfile.write(("\n     neopixels[neopixel_index][j].setRGB(red, green, blue);").encode())
                    outputfile.write(("\n     offset += 3;\n    }\n   }\n").encode())
            ...
            if scn.scn_prop.SDCardEnabled:
                ...
                outputfile.write(("\n    for(unsigned int i = 0; i < frameByteLength; i++){").encode())
                ...
                outputfile.write(("\n // If file is found, parse the header and begin playback").encode())
    ...
    def execute(self, context):
        ...
        if scn.scn_prop.WriteCacheHeader:
            ...
            if scn.scn_prop.LEDMultiSingle:
                ...
                header = header + 'S'
            header = header + str(scn.scn_prop.EndFrame-scn.scn_prop.StartFrame+1) + 'F' + str(scn.render.fps) + 'B' + str(len(outputbytes))
        ...
        for value in range(0, len(scn.my_list)):
            ...
            if scn.list_index >= 0 and len(scn.my_list) > 0 and value == scn.list_index:
                ...
                if scn.my_list[value].ParentSelect != "None" and scn.my_list[value].ChildSelect != "None":
                    ...
                    if scn.my_list[value].EnableLimits and scn.my_list[value].MotorType != "ON/OFF":
                    ...
                    if scn.my_list[value].ParentSelect != "None" and  scn.my_list[value].ChildSelect != "None" and scn.my_list[value].ShowValueDebug:
                        col= layout.column()
                    ...
                    layout.label(text="Please select an Armature and Bone for the actuator!")
    ...
    def draw(self, context):
        ...
        for value in range(0, len(scn.my_list2)):
            ...
            if scn.list_index2 >= 0 and len(scn.my_list2) > 0 and value == scn.list_index2:
                ...
                # Show LED values
                # Neopixels
                if scn.my_list2[value].LEDType == "Neopixel" or scn.my_list2[value].LEDType == "Dotstar" and scn.my_list2[value].ShowValueDebug == True:
                    ...
                    if scn.my_list2[value].LEDSingle and scn.my_list2[value].ArmatureSelect != "None" and scn.my_list2[value].RedSelect1 != "None" and scn.my_list2[value].GreenSelect1 != "None" and scn.my_list2[value].BlueSelect1 != "None":
                        ...
                        if isRGBW:
                            if scn.my_list2[value].WhiteSelect1 != "None":
                                ...
                                layout.label(text="R: " + str(redVal) + " | G: " + str(greenVal) + " | B: " + str(blueVal) + " | W: " + str(whiteVal)) if scn.my_list2[value].ShowValueDebug else 0
                        else:
                            layout.label(text="R: " + str(redVal) + " | G: " + str(greenVal) + " | B: " + str(blueVal)) if scn.my_list2[value].ShowValueDebug else 0
                    ...
                    # Individually addressable
                            ...
                            for val in range(0, len(scn.my_list2)):
                                if scn.my_list2[val].LEDType == "Single Color":
                                    ...
                                    if isNeopixelRGBW(val):
                                        from_index = from_index + 1 + scn.my_list2[val].NumLeds*4
                                    else:
                                        from_index = from_index + 1 + scn.my_list2[val].NumLeds*3
                        ...
                        layout.label(text="LED values: " + debug_string) if scn.my_list2[value].ShowValueDebug else 0

                # PWM channels
                elif scn.my_list2[value].ShowValueDebug == True and scn.my_list2[value].LEDType == "Single Color":
                    if scn.my_list2[value].ArmatureSelect != "None" and scn.my_list2[value].LEDSelect != "None" and scn.my_list2[value].LEDAngle != "None":
                        layout.label(text="PWM LED value: " + str(math.floor(interp(get_bones_rotation(scn.my_list2[value].ArmatureSelect, scn.my_list2[value].LEDSelect, scn.my_list2[value].LEDAngle), [0,90], [0,255]))))
        return
...
class MPANEL_PT_MarIOnette_OutputValuesPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.label(text="Total motor values: " + str(len(scn.my_list)) + " (" + str(len(scn.my_list) * 2) + " bytes)")
        ...
        if scn.scn_prop.SendLEDVals:
            ...
            for value in range(0, len(scn.my_list2)):
                ...
                elif scn.my_list2[value].LEDType == "Neopixel" or scn.my_list2[value].LEDType == "Dotstar":
            ...
            layout.label(text="Total LED Values: " + str(len(scn.my_list2)) + " (" + str(ledvaluenumber) + " bytes)")

        layout.label(text="Sending " + str(len(outputbytes) + 2) + " bytes" + str(outputbytes))
    ...
    def draw(self, context):
        ...
        if not scn.scn_prop.ReceivingValues:
            if writingfile:
                ...
                layout.prop(scn.scn_prop, "SDCardInternal") if scn.scn_prop.SDCardEnabled else 0
                layout.prop(scn.scn_prop, "CSPin") if not scn.scn_prop.SDCardInternal and scn.scn_prop.SDCardEnabled else 0
        ...
        if mprops.UseCache and lastframenumber != current_frame and not writingfile and current_frame <= scn.scn_prop.EndFrame and cachefilepath != 0 and not uploadingnimation:
            ...
            for i in range(0, len(scn.my_list2)):
                if scn.my_list2[i].LEDType == "Single Color":
                    ...
                    if scn.my_list2[i].NeopixelOrder == "NEO_RGBW + NEO_KHZ800" or scn.my_list2[i].NeopixelOrder == "NEO_GRBW + NEO_KHZ800" or scn.my_list2[i].NeopixelOrder == "NEO_RGBW + NEO_KHZ400" or scn.my_list2[i].NeopixelOrder == "NEO_GRBW + NEO_KHZ400":
                        isRGBW = True
